...
from models import FeaturedSpeakers
...
SESSION_WATCH_REQUEST = endpoints.ResourceContainer(
...
# - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        if not data['city']:
            data['city'] = conf.city
        if conf.city and data['city'] != conf.city:
            raise endpoints.BadRequestException("Session must share the same location with conference")
        ...
        #send session creation confirmation email
        taskqueue.add(params={'email': user.email(),
            'info': repr(request)},
            url='/tasks/send_confirmation_email'
        )
    ...
    def _addSessionToWishlist(self, request, reg=True):
        ...
        if reg:
            ...
            if wssk in prof.sessionKeysInterested:
            ...
            if wsck not in prof.conferenceKeysToAttend:
                raise endpoints.ForbiddenException(
                    "You have not registered for conference %s" % conf.name)

            prof.sessionKeysInterested.append(wssk)
            retval = True
        ...
        # delete
            ...
            if wssk in prof.sessionKeysInterested:
                ...
                # delete session
                prof.sessionKeysInterested.remove(wssk)
    ...
    @endpoints.method(SESSION_WATCH_REQUEST, BooleanMessage,
            path='conference/session/{websafeSessionKey}/add',
    ...
    @endpoints.method(SESSION_WATCH_REQUEST, BooleanMessage,
            path='conference/session/{websafeSessionKey}/delete',
    ...
    def deleteSessionFromWishlist(self, request):
        ...
        return self._addSessionToWishlist(request, reg=False)
    ...
    def getSessionsInWishlist(self, request):
        ...
        prof = self._getProfileFromUser() # get user Profile
        session_keys = [ndb.Key(urlsafe=wssk) for wssk in prof.sessionKeysInterested]
        sessions = ndb.get_multi(session_keys)

        # return set of SeesionForm objects
    ...
    #query sessions not workshop and before 7pm
    #cannot apply two inequality filters, use in memory filtering
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='querySpecificSessions',
            http_method='POST',
            name='querySpecificSessions')
    def querySpecificSessions(self, request):
        """Query for sessions."""
        results = Session.query()
        results = results.filter(Session.startTime <= 19)
        sessions = []

        for result in results:
            if result.typeOfSession != 'workshop':
